...
class Pipeline:
    ...
    def _extract_usage_tokens(self, assistant_message_obj: dict) -> Optional[dict]:
        """
        Extract usage tokens from assistant message, supporting multiple formats:
        - Grok API format: prompt_tokens, completion_tokens, reasoning_tokens
        - Open WebUI legacy format: prompt_eval_count, eval_count
        """
        ...
        # Try Grok/OpenAI standard format first
        prompt_tokens = info.get("prompt_tokens")
        completion_tokens = info.get("completion_tokens") 
        reasoning_tokens = info.get("reasoning_tokens", 0)  # Grok reasoning models
        
        # Fallback to Open WebUI legacy format
        if prompt_tokens is None:
            prompt_tokens = info.get("prompt_eval_count")
        if completion_tokens is None:
            completion_tokens = info.get("eval_count")

        # Return usage if we have both input and output tokens
        if prompt_tokens is not None and completion_tokens is not None:
            ...
            # Add reasoning tokens for Grok reasoning models
            if reasoning_tokens and reasoning_tokens > 0:
                # Store reasoning tokens in metadata for visibility
                usage_data["reasoning"] = reasoning_tokens
                # Add reasoning tokens to total output count
                usage_data["total"] = prompt_tokens + completion_tokens + reasoning_tokens
                
            self.log(f"Usage data extracted: {usage_data}")
            return usage_data
    ...
    async def outlet(self, body: dict, user: Optional[dict] = None) -> dict:
        ...
        try:
            ...
            # Extract usage tokens using improved method
            usage = self._extract_usage_tokens(assistant_message_obj)
